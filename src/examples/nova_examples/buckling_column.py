...
def define_ha():
    ...
    mode1 = ha.new_mode('1')
    mode1.set_dynamics(["x1", "2*x0-x0**3-0.2*x1+0.1"], is_linear=(True, False))
...
def define_init_states(ha):
    ...
    rv.append((ha.modes['1'], HyperBox([[-0.5, -0.5], [-0.4, -0.4]], opt=1)))
...
def define_settings():
    ...
    simu_setting = SimuSetting(model_name=model_name, horizon=horizon, init_set_bounds=[[-0.5, -0.5], [-0.4, -0.4]])
