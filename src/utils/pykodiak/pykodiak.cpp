...
extern "C"
{
    ...
    int lookupVariable(const char* name)
    {
        ...
        {
            string msg = "lookupVariable() called with unknown variable: (did you call addVaraible() first?): ";
            msg += name;
            throw runtime_error(msg);
        }
    ...
    // ndpointer(ctypes.c_double, flags="C_CONTIGUOUS"), ctypes.c_int]
    void minmax(int nonlinearExp, double* linearApprox, int linearApproxSize,
                double bias, double* bounds, int boundsRows, int boundsCols, double* rv, int rvSize)
    {
        ...
        {
            char msg[256];
            snprintf(msg, sizeof(msg), "minmax() called with linearApprox with %d entries (system has %d variables)",
        ...
        {
            char msg[256];
            snprintf(msg, sizeof(msg), "minmax() called with %dx%d bounds (expected %dx2)",
