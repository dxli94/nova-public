...
void addRowsLessEqual(glp_prob* lp, double* rhs, int rhsLen)
{
    ...
    for (int i = 0; i < rhsLen; ++i)
        glp_set_row_bnds(lp, numRows + i + 1, GLP_UP, 0, rhs[i]);  // '<=' constraint
}
             ...
             int indptrLen)
{
    ...
    else
    {
        ...
        for (int i = 0; i < indicesLen; ++i)
        {
            ...
            {
                printf(
                    "Error: csr sparse matrix has indices[%d]=%d, which is >= matrix "
                    "width (%d)\n",
                    i, indices[i], w);
                rv = 1;
                break;
             ...
             int indptrLen)
{
    ...
    else
    {
        ...
        for (int i = 0; i < indicesLen; ++i)
        {
            ...
            {
                printf(
                    "Error: csc sparse matrix has indices[%d]=%d, which is >= matrix "
                    "height (%d)\n",
                    i, indices[i], h);
                rv = 1;
                break;
                      ...
                      int numCols)
{
    ...
    if (rv == 0)
    {
        ...
        {
            printf(
                "Error: set constraints matrix out of bounds (offset was row=%d, col=%d, matrix "
...
int processSimplexResult(glp_prob* lp, int simplexRes, double* result, int resLen)
{
    ...
    else if (simplexRes != 0)
    {
        ...
        if (numCodes != numMsgs)
        {
            printf("Error: num simplex error codes(%d) is not equal to num messages (%d).\n",
                   numCodes, numMsgs);
        }
        else
        {
            ...
            printf("Error: glp_simplex returned nonzero status (%s) in minimize(): %d\n", msg,
                   simplexRes);
        }
    ...
    else
    {
        ...
        if (status == GLP_OPT)
        {
            ...
            {
                printf("Error: simplex result len (%d) is not equal to num_cols (%d)\n", resLen,
                       numCols);
                rv = -1;
            }
        }
...
int minimize(glp_prob* lp, double* direction, int dirLen, double* result, int resLen)
{
    ...
    else if (resLen != numCols)
    {
        printf("Error: resultLen(%d) is not equal to numCols(%d) in call to minimize()\n", resLen,
               numCols);
        rv = -1;
    }
    else
...
int test()
{
    ...
    else
    {
        ...
        rv = minimize(lp, direction, 1, result, 1);
